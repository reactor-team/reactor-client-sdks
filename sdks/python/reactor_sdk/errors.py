...
Every failure used to arrive as one `ReactorFFIError` carrying a sentence, which
...
class ReactorFFIError(Exception):
    """A Reactor operation failed. The base of every exception in this module.
...
class UnauthorizedError(ReactorFFIError):
...
class NotFoundError(ReactorFFIError):
    """404: no such model, session or upload."""

    code = "NOT_FOUND"


class ConflictError(ReactorFFIError):
...
class DecodeError(ReactorFFIError):
    """A reply arrived and could not be understood."""

    code = "DECODE_FAILED"


class InvalidStateError(ReactorFFIError):
...
class SessionTerminalError(ReactorFFIError):
    """The session reached a state it cannot leave. Start a new one."""

    code = "SESSION_TERMINAL"


class MessageTooLargeError(ReactorFFIError):
...
class AbortedError(ReactorFFIError):
...
#: rejected request — falls back to `ReactorFFIError` with `code` set to it.
ERROR_CLASSES: tuple[type[ReactorFFIError], ...] = (
    NetworkError,
    UnauthorizedError,
    NotFoundError,
    ConflictError,
...
_BY_CODE: dict[str, type[ReactorFFIError]] = {cls.code: cls for cls in ERROR_CLASSES}


def error_for_code(code: str | None) -> type[ReactorFFIError]:
    """The exception class for `code`, or the base class for anything unknown."""
    return _BY_CODE.get(code or "", ReactorFFIError)


def error_from_payload(raw: bytes | None) -> ReactorFFIError:
    ...
    """
    if not raw:
        return ReactorFFIError("unknown error")

    text = raw.decode(errors="replace")
    payload = _as_object(text)
    if payload is None:
        return ReactorFFIError(text)
