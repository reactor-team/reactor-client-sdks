...
describe('WebcamStream', () => {
  ...
  it('rolls back the succeeded publish when one of two tracks fails to publish', async () => {
    getUserMedia.mockResolvedValue(
      new FakeMediaStream([new FakeMediaStreamTrack('video'), new FakeMediaStreamTrack('audio')]),
    );
    ...
    currentClient().publishTrackImpl = (name) =>
      name === 'main_audio' ? Promise.reject(new Error('audio publish failed')) : Promise.resolve();
  ...
  it('republishes under new names when track props change while already published', async () => {
    let reactor: Reactor | undefined;

    function Probe({ track }: { track: string }) {
      reactor = useReactor((s) => s.internal.reactor);
      return <WebcamStream track={track} />;
    }

    const { rerender } = render(
      <Provider>
        <Probe track="main_video" />
      </Provider>,
    );
    ...
    rerender(
      <Provider>
        <Probe track="other_video" />
      </Provider>,
    );

    await waitFor(() => expect(currentClient().unpublishTrackCalls).toContain('main_video'));
    await waitFor(() => expect(currentClient().publishTrackCalls.map((c) => c.name)).toContain('other_video'));
  });
});
