...
/** Minimal fake of the `mp4box` surface `remuxFragmentedToFlat()` drives,
 *  enough to exercise the DTS-normalization math without a real fMP4
 *  fixture. `createFile()` is called twice by the module (inFile, then
 *  outFile) — the first call gets the fake parser, the second the fake
 *  writer that just records what it was asked to write. */
function createFakeMp4Box(samplesByTrack: Record<number, unknown>) {
  let createFileCalls = 0;
  const addTrackCalls: unknown[] = [];
  const addSampleCalls: Array<{ trackId: number; info: unknown }> = [];
  let nextOutTrackId = 0;

  const inFile: Record<string, unknown> = {
    setExtractionOptions: vi.fn(),
    start: vi.fn(),
    appendBuffer: vi.fn(),
    getInfo: () => ({
      tracks: Object.keys(samplesByTrack).map((id) => ({ id: Number(id), video: {}, audio: {} })),
    }),
  };

  inFile.flush = vi.fn(() => {
    (inFile.onReady as (info: { tracks: Array<{ id: number }> }) => void)({
      tracks: Object.keys(samplesByTrack).map((id) => ({ id: Number(id) })),
    });
    for (const [id, sample] of Object.entries(samplesByTrack)) {
      (inFile.onSamples as (id: number, user: unknown, samples: unknown[]) => void)(Number(id), null, [sample]);
    }
  });

  const outFile = {
    init: vi.fn(),
    addTrack: vi.fn((opts: unknown) => {
      addTrackCalls.push(opts);
      nextOutTrackId += 1;
      return nextOutTrackId;
    }),
    addSample: vi.fn((trackId: number, _data: Uint8Array, info: unknown) => {
      addSampleCalls.push({ trackId, info });
    }),
    getBuffer: vi.fn(() => ({ buffer: new ArrayBuffer(4), byteLength: 4 })),
  };

  const MP4Box = {
    createFile: vi.fn(() => (createFileCalls++ === 0 ? inFile : outFile)),
    MP4BoxBuffer: { fromArrayBuffer: (buf: ArrayBuffer) => buf },
  };

  return { MP4Box, addTrackCalls, addSampleCalls };
}
...
describe('fetchPlaylist', () => {
  ...
  it('rethrows an aborted fetch instead of wrapping it as PLAYLIST_FETCH_FAILED', async () => {
    globalThis.fetch = vi.fn().mockRejectedValue(new DOMException('Aborted', 'AbortError')) as never;

    await expect(fetchPlaylist('http://localhost/clips?x=1')).rejects.toMatchObject({ name: 'AbortError' });
  });
...
describe('downloadClipAsFile', () => {
  ...
  it('rethrows an aborted chunk fetch instead of wrapping it as CHUNK_FETCH_FAILED', async () => {
    globalThis.fetch = vi.fn().mockImplementation((url: string) => {
      if (url.includes('/clips?')) {
        return Promise.resolve(new Response(SAMPLE_MANIFEST, { status: 200 }));
      }
    ...
    await expect(downloadClipAsFile(SAMPLE_CLIP, null)).rejects.toMatchObject({ name: 'AbortError' });
  });

  it('normalizes multi-track DTS against a shared origin instead of zeroing each track independently', async () => {
    installChunkFetchMock();

    // Audio's first sample sits at 0.3s, video's at 0.5s (different
    // timescales) — the original 0.2s video-after-audio offset must survive
    // the remux instead of both tracks independently zeroing to their own
    // first sample.
    const videoSample = {
    ...
    const audioSample = {
      ...
      is_sync: true,
      timescale: 48_000,
      description: { type: 'mp4a', boxes: [] },
    };

    const { MP4Box, addSampleCalls } = createFakeMp4Box({ 1: videoSample, 2: audioSample });

    __remuxInternals.loadMp4Box = () => Promise.resolve(MP4Box as never);

    await downloadClipAsFile(SAMPLE_CLIP, null);

    expect(addSampleCalls).toHaveLength(2);
    expect((addSampleCalls[0]?.info as { dts: number }).dts).toBe(18_000);
    expect((addSampleCalls[1]?.info as { dts: number }).dts).toBe(0);
