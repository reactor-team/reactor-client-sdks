 ...
 * ahead of `mdat` — i.e. the file is implicitly faststart. Per-track decode
 * times are shifted by the first sample's DTS so the output starts at
 * `0.000000` regardless of where the clip sits on the session timeline.
 */
  ...
  return new Promise<Uint8Array>((resolve, reject) => {
    ...
    inFile.onSamples = (id, _user, samples) => {
      ...
      if (outId === undefined) {
        ...
        outId = outFile.addTrack(trackOptions);
