...
aarch64), macOS (arm64, x86_64) and Windows (x86_64), with the right native
library resolved automatically and nothing for a consumer to configure.
...
- `reactor-sdk-natives`, five classified artifacts, and
  `reactor-sdk-platform`, a POM-only aggregate for Maven builds that cannot
  read Gradle Module Metadata. Gradle consumers need only `reactor-sdk`:
  variant attributes pick the right classifier for the host.
...
- `-sources` and `-javadoc` jars for every published jar coordinate, GPG
  signed. The POM-only aggregate is exempt and has neither.
