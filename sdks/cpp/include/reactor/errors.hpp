...
/// `details` with the code its class says it has.
///
/// One class per code is the whole contract here: a caller who catches
/// `UnauthorizedError` is entitled to assume `code() == "UNAUTHORIZED"`. Nothing
/// in this SDK builds a subclass from a payload that disagrees — `make_error`
/// picks the class *from* the code — so this exists to keep the disagreement
/// unrepresentable rather than to fix one up. Everything else the payload
/// carried survives.
inline ErrorDetails with_code(ErrorDetails details, std::string_view code) {
...
#define REACTOR_DECLARE_ERROR(Name, Code)                                          \
  class Name final : public ReactorError {                                         \
   public:                                                                         \
    ...
    /* Not `using ReactorError::ReactorError`: inheriting the base's constructors  \
     * would let this class be built with any code at all, and a `Name` whose      \
     * code() says something else makes catch-based handling and code() disagree   \
     * — the one thing a class per code is for. Both of these fix the code and   \
     * copy the rest. */                                                           \
    explicit Name(ErrorDetails details)                                            \
        : ReactorError(detail::with_code(std::move(details), CODE)) {}             \
