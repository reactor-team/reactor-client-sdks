...
struct Speaker::Impl {
  ...
  void open(std::uint32_t rate, std::uint32_t channel_count) {
...
Speaker::Speaker(Track track) : impl_(std::make_shared<Impl>()) {
  ...
  subscription_ = track.on_audio([impl = impl_](const AudioFrame& frame) {
    if (!impl->running) {
      return;
    }
    impl->open(frame.sample_rate, frame.channels);
    const std::size_t dropped =
        impl->buffer.write(frame.samples, static_cast<std::size_t>(frame.num_samples));
    if (dropped != 0) {
      impl->dropped_samples.fetch_add(dropped, std::memory_order_relaxed);
    }
  });
}
...
std::uint64_t Speaker::dropped_ms() const noexcept {
  const std::uint32_t rate = impl_->sample_rate == 0 ? DEFAULT_SAMPLE_RATE : impl_->sample_rate;
  const std::uint32_t channels = impl_->channels == 0 ? 1 : impl_->channels;
...
void Speaker::submit(Samples pcm, std::uint32_t sample_rate, std::uint32_t channels) {
  ...
  impl_->open(sample_rate, channels == 0 ? 1 : channels);
  const std::size_t dropped = impl_->buffer.write(pcm.data, pcm.size);
  if (dropped != 0) {
    impl_->dropped_samples.fetch_add(dropped, std::memory_order_relaxed);
  }
}
