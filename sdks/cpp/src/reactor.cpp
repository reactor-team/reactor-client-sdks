...
///
/// Owned by the client rather than by its own completion, and this is the
/// difference that matters: `reactor_fetch_jwt` takes no handle, so
/// `reactor_destroy` neither stops the request nor waits for its completion. An
/// exchange owned only by a callback that may never come is a caller blocked in
/// `.get()` for the life of the process, and an object nothing ever frees. Held
/// here, teardown can settle the future and let it go.
...
/// rather than by dereferencing what teardown freed.
struct TokenTicket {
...
class ClientImpl : public std::enable_shared_from_this<ClientImpl> {
 ...
 public:
  ...
  /// no such promise, so the object stays alive until its completion arrives or
  /// this client goes — whichever is first.
 ...
 private:
  ...
  void destroy_handle() {
    ...
    for (auto& exchange : exchanges) {
      ...
      // alone. What matters here is the caller: nothing else is going to resolve
      // this future.
      if (exchange->op) {
        exchange->op->fail(
  ...
  /// Token exchanges in flight. Kept apart from `pending_` because their
  /// completions are not bounded by `reactor_destroy` — see `watch_exchange`.
...
void resume_connect_with_token(TokenExchange& exchange, int ok, const char* result_json,
  ...
  if (!impl) {
    // Teardown already failed this — `fail` settles once — and the exchange is
    // about to be freed with the client. Saying it again here costs nothing and
    // covers a client that expired without one.
    if (exchange.op) {
      exchange.op->fail(
          as_exception_ptr(ErrorDetails{std::string{codes::ABORTED},
                                        "the client was destroyed while exchanging the API key",
  ...
  // The completion arrived, so the client has nothing left to settle on this
  // exchange's behalf. Done before the op moves on, so teardown can never find an
  // entry whose op has already been handed to `connect`.
...
void ClientImpl::begin_connect(std::unique_ptr<Pending> op, ConnectOptions options) {
  ...
  if (needs_token) {
    ...
    // Watched before the request goes out: a completion that arrives on another
    // thread while this one is still here must find it already registered.
