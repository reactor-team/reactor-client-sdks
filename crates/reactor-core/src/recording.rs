...
async fn fetch_playlist<L: Fn() -> bool>(
    ...
    let mut remaining = readiness.timeout.map(|timeout| {
        ...
        //
        // Saturating, because the prediction arrives over a network: a nonsense one
        // is worth a wait no caller outlives, never a panic in whichever binding's
        // task this is running on. `max` also settles a NaN, which returns the
        // other operand.
        let ahead =
            Duration::try_from_secs_f64(ahead_ms.max(0.0) / 1_000.0).unwrap_or(Duration::MAX);
        timeout.saturating_add(ahead)
    });
    let mut polls = 0_u32;
...
mod tests {
    ...
    fn segments_of(
        ...
        segments_of_predicted(http, platform, jwt, timeout, 0.0, session_is_live)
    }

    /// As above, with the runtime's prediction the grace is measured from.
    fn segments_of_predicted(
        http: Arc<ScriptedHttp>,
        platform: Arc<FakeClock>,
        jwt: Option<&str>,
        timeout: Option<Duration>,
        predicted_ready_at_ms: f64,
        session_is_live: bool,
    ) -> Result<Vec<HttpRequest>, CoreError> {
    ...
    /// The prediction is what the grace is grace *past*. A clip the runtime expects
    /// two seconds from now, asked for with 1.5s of grace, has 3.5s to appear — not
    /// 1.5s, which would time out before the runtime's own estimate had passed.
    #[test]
    fn the_grace_is_measured_from_the_prediction_and_not_from_the_call() {
        let http = ScriptedHttp::playlist_answering(vec![pending(Some("1"))]);
        let clock = Arc::new(FakeClock::default());
        let predicted = clock.now_ms() + 2_000.0;

        let error = segments_of_predicted(
            http.clone(),
            clock.clone(),
            None,
            Some(Duration::from_millis(1_500)),
        ...
        assert_eq!(
            ...
            "2s of prediction plus 1.5s of grace, spent a second at a time"
        );
    }

    /// The prediction arrives over a network, so it can be anything a `double` holds.
    /// Turning one into a budget used to be an unchecked conversion and an unchecked
    /// add — either of which panics on a large enough number, inside whichever
    /// binding's task the download is running on.
    #[test]
    fn a_nonsense_prediction_is_a_long_wait_rather_than_a_panic() {
        let http = ScriptedHttp::playlist_answering(vec![pending(Some("1"))]);
        let clock = Arc::new(FakeClock::default());
        ...
        let error = segments_of_predicted(
            ...
            f64::MAX,
            // Dead, so the wait this computes ends on the session rather than on a
            // budget nothing exhausts.
            false,
        )
        .expect_err("a clip that cannot become ready is an error");
