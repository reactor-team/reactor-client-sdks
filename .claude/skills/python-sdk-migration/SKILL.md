...
| `get_remote_tracks() -> dict[str, MediaStreamTrack]` | Nothing hands you a media object. Decoded frames arrive via `on("frame", ...)` / `on("audio", ...)`, `@reactor.on_frame`, or a `Track`'s `on_frame()`/`on_raw_frame()`. |
