...
case "${1:-}" in
  ...
  build) shift; exec ./gradlew --no-daemon :reactor-core:assemble :reactor-jvm:assemble :reactor-desktop:assemble :reactor-android:assembleRelease :reactor-android:assembleDebugAndroidTest "$@" ;;
  test) shift; exec ./gradlew --no-daemon :reactor-core:test :reactor-jvm:test :reactor-desktop:test :reactor-android:testDebugUnitTest "$@" ;;
