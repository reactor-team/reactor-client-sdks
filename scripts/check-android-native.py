...
Two failures, both of which end the process rather than fail a call:

* an ELF whose LOAD segments are not 16 KB aligned will not load at all on an Android 15
  device with 16 KB pages, and
* a native library whose embedded WebRtcClassLoader name does not exist in the libwebrtc JAR
  aborts inside WebRTC bootstrap with no usable diagnostic. That is REA-6249, and it is worth
  a check rather than a comment because the name is chosen by a gn arg in another repository.
"""
